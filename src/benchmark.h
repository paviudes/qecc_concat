...
struct BenchOut
{
  ...
  3. For each metric
          ...
          (b). Variance of logical error metric values for each concatenation
  level. logvars = 2D array of shape:nmetrics x nlevels (c). Counts binned
  according to the syndrome probability and conditioned logical error metric.
                ...
                (a). Specifications of the error correcting code
                        ...
                        2. N,K,D for each code
                                int **nkd : 2D array of integers
                                        where the i-th row gives the N,K,D
   values of the i-th code.
                        3. Stabilizer syndrome signs for each code
                                int ***SS: 3D array of integers
                                        where the i-th array gives the
   stabilizer syndrome signs for the i-th code.
                        4. Logical action each code
                                int ****normalizer: 4D array of integers
                                        where the i-th array gives the logical
   action for the i-th code.
                        5. Logical action phases for each code.
                                int ***normphases: 3D array where the i-th array
   gives the phases for the i-th code. (b). Error channel
                ...
                (d). Specifications of the simulation
                        1. Type of syndrome sampling
                                int importance: string specifying the sampling
   algorithm.
                        2. Number of syndromes to be sampled at the top level,
   with breakpoints. long *stats: array of longs, where the i-th element gives
   the i-th breakpoint for the running average.
