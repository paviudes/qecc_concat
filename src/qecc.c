...
void GetFullProcessMatrix(struct qecc_t *qecc, struct simul_t *sim, int isPauli){
	...
	// PrintDoubleArray2D((sim->process)[0][0], "process[0][0]", qecc->nstabs,
	// qecc->nstabs); printf("done computing full process matrix.\n");
}
...
void ComputeSyndromeDistribution(struct qecc_t *qecc, struct simul_t *sim, int isPauli){
	...
	// PrintDoubleArray1D((sim->syndprobs), "Syndrome distribution",
	// qecc->nstabs);
}
			...
			for (l = 0; l < currentframe; l++) {
				...
				if (isPauli == 0) {
					...
					for (u = 0; u < qecc->nlogs; u++) {
						...
						for (i = 0; i < qecc->nstabs; i++)
							contrib = contrib +
						(sim->process)[u][u][i][i] * (qecc->projector)[s][i];
...
void SingleShotErrorCorrection(int isPauli, int iscorr, int frame, struct qecc_t *qecc, struct simul_t *sim, struct constants_t *consts){
	...
	// matrix of the single qubit logical channel. printf("Computing the effective
	// logical channels.\n");
	...
	for (s = 0; s < qecc->nstabs; s++) {
		...
		// printf("s = %d\n", s);
		// PrintDoubleArray2D((sim->effprocess)[s], "process", qecc->nlogs,
		// qecc->nlogs);
	}
