...
void SetOutlierProbs(double phy_infid, int dist, int level, double *outlierprobs){
	...
	outlierprobs[1] = 0.6 * (1 - Min(3 * infidelity, 1 - 1E-5));
...
void ConstructImportanceDistribution(long double *truedist, long double *impdist, int nelems, double expo){
	...
	if (expo > atol)
		for (s = 0; s < nelems; s ++)
			impdist[s] = powl(truedist[s], (long double) expo);
	else
		for (s = 0; s < nelems; s ++)
			impdist[s] = 1;
	// Normalization
...
double PowerSearch(long double *dist, int size, double *window, double *searchin){
	...
	long double pinc = 0;
	double atol = 10E-5, exponent = (searchin[0] + searchin[1])/((double)2);
	...
	while (exit == 0){
		if (fabs(searchin[0] - searchin[1]) < atol)
			...
			position = WhereInWindow((double) pinc, window);
			if (position == 0){
